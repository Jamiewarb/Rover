...
public class RoverCollect extends RoverSkeleton {
	...
	void begin() {
        ...
        } else if (getScenario() == 2 || getScenario() == 3 || getScenario() == 4) {
        	System.out.println("\n\n\nERROR: This RoverCollect should not be being used for scenarios 2, 3 or 4. Please use RoverCollectMulti.");
	...
	void poll(PollResult pr) {
		...
		case PollResult.MOVE:
			...
			try {
				...
				if (getScenario() == 2 || getScenario() == 3 || getScenario() == 4) {
					findNextResource();
				} else {
					scan(maxScan);
				}
		...
		case PollResult.COLLECT:
			...
			if ((getScenario() == 0 || getScenario() == 1 || getScenario() == 2 || getScenario() == 3) && getCurrentLoad() < maxCarry && getWorldResources() > 0) {
				try {
					collect();
				} catch (Exception e) {
					e.printStackTrace();
		...
		case PollResult.DEPOSIT:
			...
			if (getCurrentLoad() <= 0) {
				...
				try {
	                getLog().info("Scanning...");
					if (getScenario() == 2 || getScenario() == 3 || getScenario() == 0) {
						findNextResource();
					} else {
						scan(maxScan);
					}
